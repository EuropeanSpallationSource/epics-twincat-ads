...
#define RETURN_ERROR_OR_DIE(buffer,errcode,fmt, ...)   \
  ...
  extern int cmd_handle_input_line(const char *input_line,adsOutputBufferType *buffer);
  extern int getBufferSize(adsOutputBufferType *buffer);
  extern int clearBuffer(adsOutputBufferType *buffer);
  extern int cmd_buf_printf(adsOutputBufferType *buffer,const char *format, ...);
  extern int removeBytesFromBuffer(adsOutputBufferType *buffer,size_t len);
#ifdef __cplusplus
}
#endif
